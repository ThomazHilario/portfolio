...
const buttonVariants = {
    ...
    sidebarItems:"bg-transparent text-neutral-300/90 min-w-48 hover:bg-[#494BD6]/10 justify-start border-l-3 border-transparent hover:border-violet-500 max-w-40 h-10 px-2 rounded-lg",
}
...
export const Button = ({ children, className, variant = undefined, type = "button", ...props }: ButtonProps) => {
    ...
    <button 
        className={cn(
            "flex items-center justify-center text-neutral cursor-pointer transition-colors duration-300", 
            className, 
