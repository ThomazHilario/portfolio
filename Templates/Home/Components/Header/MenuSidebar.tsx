  ...
  return (
    <Drawer direction="left" open={isOpen} onOpenChange={OnToggle}>
      ...
      <DrawerContent className="flex flex-col items-center bg-neutral/95 text-default-text font-spaceGrotesk">
        <VisuallyHidden>
          <DrawerDescription>
            Componente responsavel para visualizar um menu sidebar para o
            usuário.
          </DrawerDescription>
        </VisuallyHidden>

        <DrawerHeader className="flex flex-row pb-4 gap-14 border-b border-primary/10">
          <DrawerTitle className="text-xl tracking-tighter">
            {env.PERSONAL_NAME}
          </DrawerTitle>
        ...
        <menu className="mt-8 flex flex-col items-start gap-5">
          {MENU_OPTIONS.map((option) => (
            <Button
              ...
              variant="sidebarItem"
              onClick={() =>
                handleScrollSection(() => scrollSection(option.id))
              }
